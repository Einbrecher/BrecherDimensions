...
public class DimensionRegistrar {
    ...
    public void createDimensionsOnServerStart(MinecraftServer server) {
        ...
        List<? extends String> enabledDimensions = BrecherConfig.enabledDimensions.get();
        
        for (String baseDimStr : enabledDimensions) {
            try {
                ResourceLocation baseDim = ResourceLocation.tryParse(baseDimStr);
                if (baseDim == null) {
                ...
                if (explorationLevel != null) {
                    ...
                    if (!explorationKey.equals(baseKey)) {
                        ...
                        // Pre-generate spawn chunks if enabled
                        SpawnChunkPreGenerator.preGenerateSpawnArea(explorationLevel);
                    } else {
                        LOGGER.warn("Exploration dimension creation returned base dimension for {}. " +
                            "This may be due to Forge limitations.", baseDim);
                    }
                } else {
    ...
    public List<String> getDimensionInfo() {
        ...
        dimensionMappings.forEach((baseLoc, exploreKey) -> {
            Long seed = dimensionSeeds.get(exploreKey);
            info.add(String.format("%s -> %s (seed: %d)", 
                baseLoc, exploreKey.location(), seed != null ? seed : 0L));
        });
        return info;
    }
